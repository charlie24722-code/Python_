...
(function () {
  ...
  var CODIGO_WORKER = [
    ...
    '    var enviados = 0;',
    '    function enviar(tipo, s) {',
    '      if (enviados > LIMITE_SALIDA) return;',
    '      enviados += s.length + 1;',
    '      if (enviados > LIMITE_SALIDA) { self.postMessage({ id: d.id, tipo: "err", texto: "\\n… salida recortada: el programa imprimió demasiado. ¿Hay un print dentro de un bucle infinito?\\n" }); return; }',
    '      self.postMessage({ id: d.id, tipo: tipo, texto: s + "\\n" });',
    '    }',
    '    p.setStdout({ batched: function (s) { enviar("out", s); } });',
    '    p.setStderr({ batched: function (s) { enviar("err", s); } });',
    '    var modulos = [];',
